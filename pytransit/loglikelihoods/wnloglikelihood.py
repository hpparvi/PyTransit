...
class WNLogLikelihood(LogLikelihood):
    def __init__(self, tm, obs, err):
        ...
        if self.tm.backend == 'jax':
            ...
            def loglike(k, t0, p, a, i, e, w):
                fmod = model(k, t0, p, a, i, e, w)
                return - 0.5*self.obs.size*jnp.log(2*jnp.pi) - 0.5*jnp.sum(jnp.log(self.err**2)) - 0.5*jnp.sum((self.obs - fmod)**2 / self.err**2)

            if self.tm.return_grad:
                self._loglikelihood = jax.jit(jax.value_and_grad(loglike, argnums=tuple(range(7))))
            else:
